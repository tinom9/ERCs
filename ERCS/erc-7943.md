...
interface IERC7943Fungible is IERC165 {
    ...
    /// @param canSend Whether account can send tokens or not.
    /// @param canReceive Whether account receive tokens or not.
    error ERC7943CannotTransact(address account, bool canSend, bool canReceive);
    ...
    /// @return canSend True if the account can send tokens, false otherwise.
    /// @return canReceive True if the account can receive tokens, false otherwise.
    function canTransact(address account) external view returns (bool canSend, bool canReceive);
...
Note that while the mentioned base token standards are listed as well-known and widely adopted examples, this interface is designed to support other existing or future token standards. An example of this is [ERC-6909](./eip-6909.md) which is compatible with the `IERC7943MultiToken` interface.
...
- `canTransfer`:
    ...
    - MUST perform a `canTransact` check on the `from` and `to` parameters. Note that [ERC-3643](./eip-3643.md) does not perform a `canTransact` check within `canTransfer` as this standard requires.
    ...
    - SHOULD NOT return false even if token-specific, non-permissioned validations would prevent a given transfer from succeeding. For example, if the account does not hold enough balance to execute a transfer but there are no permissioned rules preventing it, `canTransfer` SHOULD return true.
...
- **Public transfers** (`transfer`, `transferFrom`, `safeTransferFrom`, etc.) MUST NOT succeed in cases where `canTransfer` or `canTransact` would return `false` for either one or both `from` and `to` addresses.
...
- **Flexible compliance**: Provides standard view functions (`canTransact`, `canTransfer`, `getFrozenTokens`) for compliance checks without dictating _how_ those checks are implemented internally by the token contract. This allows diverse compliance strategies.
- **Compatibility**: Designed as an interface layer compatible with existing base standards like [ERC-20](./eip-20.md), [ERC-721](./eip-721.md), and [ERC-1155](./eip-1155.md). Implementations extend from [ERC-7943](./eip-7943.md) alongside their base standard interface. Additionally, with the adopted naming conventions, backward compatibility is maximized with already existing standards like [ERC-3643](./eip-3643.md) and [ERC-7518](./eip-7518.md).
...
Reference implementations of uRWA for [ERC-20](../assets/eip-7943/contracts/uRWA20.sol), [ERC-721](../assets/eip-7943/contracts/uRWA721.sol), and [ERC-1155](../assets/eip-7943/contracts/uRWA1155.sol) token implementations are provided in the assets folder. They use the OpenZeppelin library and include an account whitelist and enumerable role-based access control. These examples are provided for educational purposes only and are not audited.
