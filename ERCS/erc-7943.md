...
- `canTransfer`:
    ...
    - MUST return true even if token-specific, non-permissioned validations would prevent a given transfer from succeeding. For example, if the account does not hold enough balance to execute a transfer but there are no permissioned rules preventing it, `canTransfer` MUST return true.
