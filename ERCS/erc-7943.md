...
- **Front-running of the `forcedTransfer` and `setFrozenTokens` function**: Both functions are susceptible to front-running, similar to the `approve` function of the [ERC-20](./eip-20.md). Furthermore, if the suggestion of allowing freezing more than what an account owns is not followed, any account may be incentivized to front-run attempts to freeze its balance when receiving new funds. Additional features to gradually increment or decrement the frozen status MAY be considered for implementation.
